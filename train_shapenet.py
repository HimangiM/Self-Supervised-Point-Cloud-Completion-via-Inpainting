...
from data_util_octants_shapenet_train import lmdb_dataflow
from data_util_octants_shapenet_test import lmdb_dataflow_test
...
def pc_sensor_to_canonical(points_sensor, pred_pose):
    points_mean_centered = points_sensor #- np.mean(points_sensor, axis=0)
    points_mean_centered_4 = np.concatenate((points_mean_centered, np.ones((points_mean_centered.shape[0], 1))), axis=1)
    points_can = np.matmul(points_mean_centered_4, pred_pose.T)

    x_min, y_min, z_min = np.min(points_can[:, :3], axis=0)
    x_max, y_max, z_max = np.max(points_can[:, :3], axis=0)

    x_center = (x_min + x_max)/2
    y_center = (y_min + y_max)/2
    z_center = (z_min + z_max)/2

    centers = np.array([x_center, y_center, z_center])

    points_trans = points_can[:, :3] #- centers
...
def return_octants(points, pts_threshold=128, fuzzy_threshold=0):
    ...
    x_center, y_center, z_center = np.mean(corners, 0)
...
def test(args):
    ...
    # print ('checkpoint here:', args.checkpoint)
    ...
    chamfer_dict['missing'] = []
    chamfer_dict['present'] = []

    if args.id_data == 'car':
        data_ids = ['56d463162ff5352cbd835ce3c63f4d10_1',
                    '501ac8493044eff04d44f5db04bf14b8_1',
                    'c30bf6d1ae428497c7f3070d3c7b9f30_1',
                    'dd0817b9757b74c75d3a87a5c4dd9ce4_4',
                    '501ac8493044eff04d44f5db04bf14b8_2',
                    '36b211593bfa7c2f7f33a9b0659de0d7_2',
                    'c9991032ff77fe8552470de2774d6099_2',
                    ]

    if args.id_data == 'chair':
        data_ids = ['70f1f85d47c970bb78dd615a59de5f05_3',
                    '7035d480f6bda22938b39a90ee80e328_1',
                    '9df79431f16cad05116162acefe23592_0',
                    '9bb6d3d76d4f5ba94b3c42e318f3affc_1',
                    'ea762c2f1c6e966be464faca7dae7fdd_3',
                    '98d20227789f5c9a5af29473330f42b1_0',
                    'b8e5ce05c382b9a3f51f77a6d7299806_1',
                    '3374905da20ad76f3c9c1464e55d580_3',
                    'b59735c72cf9af0c17b431cae0dd70ed_1']


    if args.id_data == 'plane':
        data_ids = ['952c5b41a44f6f3166cf1b4a8fc3914e_3',
                    '6cdc9acb022b2d7d98aeb62a3dfc01d8_2',
                    '37cbf91baa58d6577dc01d89d8a05b74_2',
                    '236932c4f4b2f557f5c7319b71bdce6e_3',
                    '55ed35a03b08a8f44f95630cc18536e0_3',
                    '3a7e396786751f544c3a35cee92bb95b_0',
                    '26950e05ed379413bbf3143b1cb6076a_1',
                    '3a18489f9615a350e768735f27170bc4_0',
                    '29b92db18649b64e959a8a7d5a8a5077_1',
                    'aad69c419b27d6744d8e7d3c15c98c3b_3']

    heatmap_chair_ids = ['7035d480f6bda22938b39a90ee80e328_1',
                         '70f1f85d47c970bb78dd615a59de5f05_1',
                         '70f1f85d47c970bb78dd615a59de5f05_3',
                         '70f1f85d47c970bb78dd615a59de5f05_4',
                         '9bb6d3d76d4f5ba94b3c42e318f3affc_1',
                         '9df79431f16cad05116162acefe23592_0']
    ...
    for e in range(num_eval_steps):
        ...
        if e < 10:
            np.savez(os.path.join(args.results_dir, 'test_output_sep_decoder_{}'.format(e)),
                         output = output_concat, gt = gt_original_pcn,
                         inputs = sep_inputs, ids = ids, gt_network = gts_pcn,
                         output_pcn = fine_output[1], output_octants = fine_output[0])


        inputs_l = np.split(inputs_pcn[0], np.cumsum(npts_pcn)[:-1])


        for bv in range(args.batch_size):
            ids_bv = ids[bv]
            input_points = inputs_l[bv]
            gt_points = gt_original_pcn[bv].reshape(-1, 3)
            output_points = output_concat[bv, :, :].reshape(-1, 3)

            rot_matrix = [[1,       0,          0],
                  [0, np.cos(89.5), -np.sin(89.5)],
                  [0, np.sin(89.5), np.cos(89.5)]]

            output_points = np.matmul(output_points, rot_matrix)
            ...
            precision_val = np.mean(min_dist_1_np) * 100
            coverage_val = np.mean(min_dist_0_np) * 100
            ...
            # to count present and missing octants
            input_rot_points = np.matmul(input_points, rot_matrix)
            octants_l = return_octants(input_rot_points)
            missing = 0
            present = 0
            for ol in octants_l:
                if np.sum(ol) == 0:
            ...
            chamfer_dict['present'].append(present)
            chamfer_dict['missing'].append(missing)
            print ('Present, missing', present, missing)

            # if ids_bv in heatmap_chair_ids:
            #     print (ids_bv)
            #     np.savez(os.path.join(args.results_dir, 'test_output_{}'.format(ids_bv)),
            #              output = output_points, gt = gt_points,
            #              inputs = input_points, ids = ids)

            # ids_l = ['103402871ed03ed117a54d13fb550a39_0', '103402871ed03ed117a54d13fb550a39_1',
            #          '103402871ed03ed117a54d13fb550a39_2', '103402871ed03ed117a54d13fb550a39_3',
            #          '103402871ed03ed117a54d13fb550a39_4']
            # if ids_bv in ids_l:
            #     print (ids_bv)
            #     np.savez(os.path.join(args.results_dir, 'test_output_{}'.format(ids_bv)),
            #              output = output_points, gt = gt_points,
    ...
    for step in range(init_step+1, args.max_step+1):
        ...
        if step % args.steps_per_visu_save == 0 or step == args.max_step:
            ...
            np.savez(os.path.join(args.results_dir, 'results_{}'.format(step)),
                     ...
                     complete_outputs = fine_output[0],
                     complete_pcn = fine_output[1], inputs_lmdb = inputs_octants,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--id_data', type=str)
    parser.add_argument('--checkpoint', type=str, default='model-400000.ckpt')
