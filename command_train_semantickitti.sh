...
python train_semantickitti.py \
    ...
    --num_parts 8 \
    --steps_per_print 100 \
    --results_dir results/results_kitti \
    --steps_per_visu_save 25000 \
    --steps_per_save 50000 \
    --steps_per_eval 500000 \
    --lmdb_train kitti/kitti_train.lmdb \
    --lmdb_valid kitti/kitti_train.lmdb \
    --lmdb_test kitti/kitti_test.lmdb \
    --encoder_dropout 0.3 \
    --prob_zero_start 2 \
    --prob_zero_end 7 \
    --fuzzy_boundary 0.02 \
    --slurm_output_file output_logs/output_kitti.txt \
