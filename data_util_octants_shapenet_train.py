...
class BatchData(dataflow.ProxyDataFlow):
    ...
    def _aggregate_batch(self, data_holder, use_list=False):
        ...
        # PCN data
        inputs_pcn = [resample_pcd(resample_pcd(pc_sensor_to_canonical(x[1], x[4]), 2048), self.input_size_pcn) if pc_sensor_to_canonical(x[1], x[4]).shape[0] > self.input_size_pcn else
...
def lmdb_dataflow(lmdb_path, batch_size, input_size_octants, output_size_octants,
