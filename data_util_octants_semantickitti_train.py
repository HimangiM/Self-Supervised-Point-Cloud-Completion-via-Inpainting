...
class BatchData(dataflow.ProxyDataFlow):
    ...
    def _aggregate_batch(self, data_holder, use_list=False):
        ...
        for e, x in enumerate(data_holder):
            octants = return_octants(x[0], pts_threshold=1, fuzzy_threshold=self.fuzzy_boundary)
            missing_octant_ = []
        ...
        # PCN data
        inputs_pcn = [resample_pcd(resample_pcd(x[0], 2048), self.input_size_pcn) if x[0].shape[0] > self.input_size_pcn else x[0]
            for e, x in enumerate(data_holder)]
