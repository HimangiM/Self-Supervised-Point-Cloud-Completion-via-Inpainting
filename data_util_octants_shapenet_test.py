...
class BatchData(dataflow.ProxyDataFlow):
    ...
    def _aggregate_batch(self, data_holder, use_list=False):
        ...
        ids = np.stack([x[0] for e, x in enumerate(data_holder) if e in idx_data_holder])
        npts = [[] for _ in range(8)]
        resampled_octants = [[] for _ in range(8)]
        gts = [[] for _ in range(8)]
        missing_octants = []
        for e, x in enumerate(data_holder):
            if e in idx_data_holder:
                octants = return_octants(pc_sensor_to_canonical(x[1], x[4]), pts_threshold=4, fuzzy_threshold=self.fuzzy_boundary)
                missing_octant_ = []
        ...
        # PCN data
        inputs_pcn = [resample_pcd(pc_sensor_to_canonical(x[1], x[4]), self.input_size_pcn) if pc_sensor_to_canonical(x[1], x[4]).shape[0] > self.input_size_pcn else
                      pc_sensor_to_canonical(x[1], x[4]) for e, x in enumerate(data_holder) if e in idx_data_holder]
        inputs_pcn = np.expand_dims(np.concatenate([x for x in inputs_pcn]), 0).astype(np.float32)
        npts_pcn = np.stack([pc_sensor_to_canonical(x[1], x[4]).shape[0] if pc_sensor_to_canonical(x[1], x[4]).shape[0] < self.input_size_pcn else self.input_size_pcn
            for e, x in enumerate(data_holder) if e in idx_data_holder]).astype(np.int32)
        gts_pcn = np.stack([resample_pcd(pc_sensor_to_canonical(x[1], x[4]), self.gt_size_pcn) for e, x in enumerate(data_holder) if e in idx_data_holder]).astype(np.float32)
        gt_original_pcn = np.stack([resample_pcd(x[5], self.gt_size_pcn) for e, x in enumerate(data_holder) if e in idx_data_holder]).astype(np.float32)
...
def lmdb_dataflow_test(lmdb_path, batch_size, input_size_octants, output_size_octants,
    ...
    df = BatchData(df, batch_size//4 * 5, input_size_octants, output_size_octants,
